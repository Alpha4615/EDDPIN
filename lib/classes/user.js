...
/**
 * Generates the nick!ident@host mask for the user
 * @returns {String}
 * @todo implement fakehost support.
 */
user.prototype.getMask = function () {
    return util.format("%s!%s@%s", this.nickname, this.ident, this.host);
};
 ...
 * @returns {Boolean}
 */
user.prototype.isExceptedOn = function (channel) {
    if (!core.isType_channel(channel)) {
        throw new TypeError("Valid channel instance expected");
    }
 ...
 * @returns {Boolean}
 */
user.prototype.canJoin = function (channel) {
    if (!core.isType_channel(channel)) {
        throw new TypeError("Valid channel instance expected");
    }
...
user.prototype.quit = function () {
    for (var key in this.channels) {
        this.channels[key].channel.userPart(this);
    }
};
