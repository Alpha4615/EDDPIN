    ...
    if (core.isChannel(target) === true) {
        ...
        for (x = 0; x < paramModes.length; x++) {
            ...
            if (cursor !== '+' && cursor !== '-') {
                ...
                if (isUser === true) {
                    ...
                    if (token === 'OM') {
                        // Was this an override by an oper?
                        var alertEvent = new core.generic.event();
                        eventObj.data.isOpMode = true;
                        alertEvent.actor = eventObj.actor;
                        alertEvent.victim = eventObj.victim;
                        alertEvent.channel = eventObj.channel;

                        alertEvent.data.subEvent = eventObj;

                        core.event.trigger('channelUserStatusChange_operOverride', alertEvent);
                    }
        ...
        if (didchanModeChange === true) {
            ...
            if (token === 'OM') {
                ...
                core.event.trigger('channelModeChange_operOverride', alertEvent);
            }
