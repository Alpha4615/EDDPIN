...
class Parser {
	...
	parse (parseObj) {
		...
		if (typeof this.#_tokenMap[parseObj?.token] !== "function") {
			return false;
		}
		this.#_tokenMap[parseObj?.token].call(this, parseObj);
	
		return true;
	};

	handleBurst (parseObj) { // channel introduced by netburst
		let core = this.#_core;
		...
		var channel = (core.getChannelByName(chanName) === undefined ? new Channel(chanName, creationTime) : core.getChannelByName(chanName));
		var workingUser;
		var userSplit;
		var modes;
		var foundUser;
		...
		if (channel.modes !== '') {
			...
			for (var paramIterator = 0; paramIterator < paramModes.length; paramIterator++) {
				...
				channel.setParameterModeValue(paramModes[paramIterator], spaceDelimited[4 + paramIterator + 1]);
			}
		}
	...
	handleNewServer_uplink (parseObj) {
		var core = this.#_core;
		var eventObj = parseObj.eventObj;
	...
	handleChannelCreate (parseObj) { // user is creating a channel (first one to join it after emptied)
		let core = this.#_core;
		var spaceDelimited = parseObj.spaceDelimited;
		var eventObj = parseObj.eventObj;
		...
		for (var x = 0; x < channelSplit.length; x++) {
			channel = new Channel(channelSplit[x]);
			core.setChannel(channel);
	...
	handleChannelPart (parseObj) { // user parts channel
		let core = this.#_core;
		var spaceDelimited = parseObj.spaceDelimited;
		var eventObj = parseObj.eventObj;
		var string = parseObj.string;
	...
	handleN (parseObj) { /// new user or nickname change
		var core = this.#_core;
		var eventObj = parseObj.eventObj;
		...
		if (spaceDelimited.length === 4) { // this is a user changing their nickname!
			...
			var newUser = new User(optionsObject, core);
	
			core.setUser(newUser);
	...
	handleEndofBurst (parseObj) { // end of burst (supposedly sync'd to network)
		var core = this.#_core;
		var server = parseObj.originatingServer;
					...
					if (isUser === true) {
						...
						if (plusorminus === '+') {
							...
							if (cursor === 'b') {
								...
								channel.modeParams[cursor] = undefined;
							}
						}
	...
	handleTopic (parseObj) { // topic is changed
		var core = this.#_core;
		var eventObj = parseObj.eventObj;
		var spaceDelimited = parseObj.spaceDelimited;
		var string = parseObj.string;
