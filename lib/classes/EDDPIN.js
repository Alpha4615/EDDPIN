...
EDDPIN.prototype.parse = function (data) {
    ...
    for (x = 0; x < incomingData.length; x++) {
        ...
        if (token === 'S') {
            ...
            channel = (core.getChannelbyName(chanName) === false ? new core.generic.channel() : core.getChannelbyName(chanName));
            var workingUser;
            var userSplit;
            var modes;
                    ...
                    if (cursor !== '+' && cursor !== '-') {
                        ...
                        if (isUser === true) {
                            ...
                            if (typeof targetMap === 'undefined')
                                throw new Error(value + " Not on Channel");
                            if (plusorminus === '+') {
            ...
            if (string.substr(0, 1) == '' && string.substr(string.length - 1, 1) == '') {
                ...
                eventObj.data.text = string.substr(1, string.length - 2).toUpperCase();
            }
