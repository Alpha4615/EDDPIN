...
const EventMgr = require("./eventRegister.js");
...
class EDDPIN {
	...
	get serverNumeric () { return this.#_serverNumeric }
	...
	getTimestampUTC () {
		...
		return Math.floor(d1.getTime() / 1000) + this.timeOffset;
	};
	 ...
	 */
	resolveModes (oldModes, modeAdjustment) {
		var addMode = true;
		var newModes = oldModes.split('');
	 ...
	 * @returns {{standardModes: string, paramModes: string}|*}
	 * @see {separateChannelModes}
	 * @see {separateUserModes}
	 */

	separateParamModes (modeList, paramList) {
		...
		returnObj = {standardModes: newSplit_changes.join(''), paramModes: newParamModes};
	 ...
	 */
	getUserByNumeric (numeric) {
		return this.#_users.get(numeric);
	};
	...
	getUserByNickname (nickname) {
		...
		for (const [key, value] of this.#_users) {

			if (value.nickname.toLowerCase() == nickname) {
				return value;
			}
		}
	...
	/**
	 * Determines if the input is an instance of the channel class
	 * @param test {*}
	 * @returns {boolean}
	 */
	isType_channel (test) {
		return test instanceof this.generic.channel;
	};

	/**
	 * Determines if the input is an instance of the user class
	 * @param test
	 * @returns {boolean}
	 */
	isType_user (test) {
		return test instanceof User || test instanceof this.generic.user;
	};

	/**
	 * Determines if the input is an instance of the user class AND NOT the descendant pseudo-user class
	 * @param test
	 * @returns {boolean}
	 */
	isType_user_strict (test) {
		return (this.isType_user(test) && !this.isType_pseudouser(test));
	};

	/**
	 * Determines if the input is an instance of the pseudo user class
	 * @param test
	 * @returns {boolean}
	 */
	isType_pseudouser (test) {
		return test instanceof this.generic.pseudouser;
	};

	/**
	 * Determines if the input is an instance of the event class
	 * @param test
	 * @returns {boolean}
	 */
	isType_event (test) {
		return test instanceof this.generic.event;
	};

	/**
	 * Determines if the input is an instance of the server class
	 * @param test
	 * @returns {boolean}
	 */
	isType_server (test) {
		return test instanceof this.generic.server;
	};

	isType_module (test) {
		return test instanceof this.generic.module;
	};
	...
	setChannel (channel) {
		...
		this.#_channels.set(channel.nameSafe, channel);
	}
	...
	destroyChannel (channel) {
		...
		this.#_channels.delete(nameSafe);
	};
	...
	sequence2CharString (num, length) {
	 ...
	 */
	generateNumeric () {
		return this.#_serverNumeric + this.sequence2CharString(Object.keys(this.myUsers).length, 3);
	};
	 ...
	 * @param module {module} The module to attach this pseudo user to
	 ...
	 */
	createPseudoUser (module, optionsObj) {
	...
	setUser (user) {
		...
		this.#_users.set(user.numeric, user);
	}
	/**
	...
	destroyUser (user) {
		...
		// remove reference from core
		this._users.delete(numeric);
	};

	logger = require('./logger.js');
	
	highlightProtocolString (originalString) {
		// Make the string
	...
	parse (data) {
		...
		for (x = 0; x < incomingData.length; x++) {
			...
			originatingServer = this.getServerByNumeric(spaceDelimited[0].substr(0, 2));
			eventObj.originatingServer = originatingServer;
			eventObj.raw = {
				full: emittedString,
	 ...
	 */
	start () {
		this.#_socket = new this.net.Socket();
		var core = this;
		core.loadModule_group('appInit');

		this.#_socket.connect(core.config.uplink.port, core.config.uplink.server,
			function () {
				console.log("Connected!".green.bold);
				core.loadModule_group('uponUplinkConnection');
				var sendString = util.format("SERVER %s 1 %s %s J10 %s +hs :%s", core.config.me.serverName, core.getTimestampUTC(), core.getTimestampUTC(), core.serverNumeric + ']]]', core.config.me.description);
	...
	get channels() {
		let returnVal = [];
		for (const [key, value] of this.#_channels.entries()) {
	...
	get users() {
		let returnVal = [];
		for (const [key, value] of this.#_users.entries()) {
			returnVal.push(value);
		}
