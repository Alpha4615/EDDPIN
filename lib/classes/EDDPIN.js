...
var EDDPIN = function (config) {
    this.config = config;
    this.serverNumeric = this.config.me.numeric;
    this.generic = {};
...
EDDPIN.prototype.generateNumeric = function () {
    ...
    do {
        ...
        newNumeric = this.serverNumeric + text;
    } while (this.myUsers[newNumeric] !== undefined);

    return newNumeric;
};

EDDPIN.prototype.createPseudoUser = function (module, nickname, GECOS, ident, host, fakehost, account, usermodes) {
    var numeric = this.generateNumeric();
    var created = new this.generic.user(nickname, numeric, GECOS, ident, host, fakehost, account, usermodes);
    created.isPseudo = true;
...
EDDPIN.prototype.start = function () {
        ...
        for (x = 0; x < incomingData.length; x++) {
            ...
            if (token === 'S') {
                ...
                if (spaceDelimited.length === 4) { // this is a user changing their nickname!
                    ...
                    var newUser = new core.generic.user(regexMatches[1], //nickname
                        regexMatches[11], // numeric
                        regexMatches[12], // GECOS
                        regexMatches[3], // ident
                        regexMatches[4], // host
                        regexMatches[9], // fake host
                        regexMatches[7], // account
                        regexMatches[5]); //user modes
                ...
                for (userIndex = 0; userIndex < userRaw.length; userIndex++) {
                    ...
                    channel.userJoin(foundUser, modes);
    ...
    this.socket.on("close", function () {
        ...
        core.event.trigger('disconnected');
    });
};
